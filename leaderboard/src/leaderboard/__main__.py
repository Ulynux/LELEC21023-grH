...
def create_app() -> Flask:
    ...
    if os.environ["FLASK_RUN_HOST"].lower() in ["http://lelec210x.sipr.ucl.ac.be", "0.0.0.0"]:
        socketio = SocketIO(
            app,
            **socketio_kwargs,
        )
...
def serve(_open: bool = False, _open_doc: bool = False):
    ...
    if _open and host in ["http://lelec210x.sipr.ucl.ac.be", "0.0.0.0"]:
        Timer(
            1, lambda: webbrowser.open(f"http://{host}:{port}/lelec210x/leaderboard")
        ).start()

    if _open_doc and host in ["http://lelec210x.sipr.ucl.ac.be", "0.0.0.0"]:
        Timer(
            1,
